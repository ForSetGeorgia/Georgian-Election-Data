# encoding: UTF-8
class UpdateExportText < ActiveRecord::Migration
  def up
    PageTranslation.transaction do
      p = PageTranslation.where(:id => 5)
      p.first.description = 'You can export the map in two ways.

  1 - PNG/SVG. You can click on the button labeled \'PNG\' or \'SVG\' to download an image of the map. SVG is similar to an image (PNG), but it contains layers that you can interact with. For example, you can change colors, add text, move shapes, etc. You can use a free application called Inkscape to edit these SVG files and save them into different formats like PDF and PNG.
...
1 - PNG/SVG. რუკის სურათად გადმოსაწერად დააჭირეთ მარჯვენა ქვედა კუთხეში ღილაკს "PNG" ან "SVG". SVG-ით იმავე გამოსახულებას (PNG) მიიღებთ, რაც რუკაზეა, ამასთან მას აქვს დამატებითი ფუნქციები რითაც შეგიძლიათ ისარგებლოთ. მაგალითად, შეგიძლიათ შეცვალოთ ფერები, დაამატოთ ტექსტი, გადაანაცვლოთ საზღვრები და ა.შ. ამისათვის შეგიძლიათ გამოიყენოთ უფასო აპლიკაცია სახელად "Inkscape", განახორციელოთ ცვლილებები SVG ფაილში და შეინახოთ სხვადასხვა ფორმატში როგორიცაა PDF ან PNG.
