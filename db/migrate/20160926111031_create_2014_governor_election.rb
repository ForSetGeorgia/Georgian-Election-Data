class Create2014GovernorElection < ActiveRecord::Migration
...
#    ['სახალხო მოძრაობა', 'სახალხო მოძ.', 'ხმების გადანაწილება, სახალხო მოძრაობა (%)'],
  ...
  def up
    ...
    Event.transaction do
      ...
      if event.present?
        ...
        locale = I18n.locale
        I18n.locale = :en
        file_path = "#{Rails.root}/db/load data/2014/upload_2014_official_gamgebeli.csv"
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2014-06-15', event_translations: {name: '2014 Governor Election'}).first
      if event.present?
