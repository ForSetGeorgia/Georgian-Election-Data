class Create2016ParlMajorElection < ActiveRecord::Migration
...
NEW_PARTIES = {
  ...
  def up
    clone_event_id = 32 # 2012 parl major
    clone_ind_event_id = 32 # 2012 parl major
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2016-10-08', event_translations: {name: '2016 Parliamentary - Majoritarian'}).first
      if event.present?
