class Create2017Governor < ActiveRecord::Migration
...
NEW_PARTIES = {
  ...
  def up
    clone_event_id = 40 # 2014 governor
    clone_ind_event_id = 40 # 2014 governor
      ...
      if event.present?
        ...
        event.event_translations.each do |trans|
          ...
          trans.description = if trans.locale == 'en'
            'The results of the October 21, 2017 election. Governors are elected for four year terms.'
          else
            '2017 წლის 21 ოქტომბერის გამგებლების არჩევნების შედეგები. გამგებლები არჩეულია 4 წლის ვადით.'
          end
        end
        event.save
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2017-10-21', event_translations: {name: '2017 Governor Election'}).first
      if event.present?
