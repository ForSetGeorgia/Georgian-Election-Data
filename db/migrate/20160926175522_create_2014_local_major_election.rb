class Create2014LocalMajorElection < ActiveRecord::Migration
...
#    ['სახალხო მოძრაობა', 'სახალხო მოძ.', 'ხმების გადანაწილება, სახალხო მოძრაობა (%)'],
  ...
  def up
    clone_event_id = 15 # 2010 local major
    ...
    Event.transaction do
      ...
      if event.present?
        ...
        # update values
        event.shape_id = 60412
        event.event_translations.each do |trans|
          trans.name = trans.name.gsub('2010', '2014')
          trans.name_abbrv = trans.name_abbrv.gsub('2010', '2014')
        ...
        file_path = "#{Rails.root}/db/load data/2014/Indicator_Names_Scales_for_2014_Local_Election_-_Majoritarian.csv"
        ...
        # load the data
        ################################################
        file_path = "#{Rails.root}/db/load data/2014/upload_2014_official_local_major.csv"
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2014-06-15', event_translations: {name: '2014 Local Election - Majoritarian'}).first
      if event.present?
