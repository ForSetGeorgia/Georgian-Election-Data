...
class AddNewBiElecIndicators < ActiveRecord::Migration
  def up
    ...
    # Ioseb Manjavidze
    core = CoreIndicator.create(:indicator_type_id => 2, :number_format => '%', :parent_id => init_grp_id)
  ...
  def down
    ...
    CoreIndicatorTranslation.where(:id => inds.select{|x| x.id}.delete_all
    CoreIndicator.where(:id => inds.select{|x| x.core_indicator_id}.delete_all
  end
end
