class Create2014LocalPartyElection < ActiveRecord::Migration
...
NEW_PARTIES = {
  ...
  def up
    clone_event_id = 16 # 2010 local party list
    ...
    Event.transaction do
      ...
      if event.present?
        ...
        locale = I18n.locale
        I18n.locale = :en
        file_path = "#{Rails.root}/db/load data/2014/upload_2014_official_local_party.csv"
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2014-06-15', event_translations: {name: '2014 Local Election - Party List'}).first
      if event.present?
