class Create2017GovernorRunoff < ActiveRecord::Migration
...
NEW_PARTIES = {
  ...
  def up
    clone_event_id = 44 # 2014 governor runoff
    clone_ind_event_id = 44 # 2014 governor runoff
    clone_core_ind_id = 59 #unm

    Event.transaction do
      event = Event.clone_from_event(clone_event_id, '2017-11-12')

      if event.present?
        puts "event created with id of #{event.id}"

        # update values
        event.shape_id = 93196
        event.event_translations.each do |trans|
          if trans.locale == 'en'
            trans.name = '2017 Self-Governing Community Mayor Runoff Election'
            trans.name_abbrv = '2017 Self-Governing Community Mayor Runoff'
            trans.description = 'The results of the November 12, 2017 runoff election for Self-Governing Community Mayor. For 5 mayor races, no candidate received more than 50% of the vote on October 21, 2017 so a runoff was held. Mayors of self-governing communities are elected for four year terms.'
          else
            trans.name = '2017 წლის თვითმმართველი თემის მერის არჩევნები მეორე ტური '
            trans.name_abbrv = '2017 წლის თვითმმართველი თემის მერის მეორე ტური '
            trans.description = '2017 წლის 11 ნოემბერის თვითმმართველი თემის მერი არჩევნების მეორე ტურის შედეგები. 2017 წლის 21 ოქტომბერის 5 ქალაქში ხმების 50%-ზე მეტის მიღება ვერც ერთმა კანდიდატმა შეძლო, რის გამოც მეორე ტური დაინიშნა. თვითმმართველი თემის მერები არჩეული არიან 4 წლის ვადით.'
          end
        end
        event.save
        ...
        end_date = '2017-12-15'
        data_at = '2017-11-13 18:00:00 +0400'
  ...
  def down
    Event.transaction do
      event = Event.includes(:event_translations).where(:event_date => '2017-11-12', event_translations: {name: '2017 Self-Governing Community Mayor Runoff Election'}).first
      if event.present?
