...
select
 district_id, 
`Free Georgia count`
from `2012 election parl party list - districts`
;
...
select
 district_id, 
`Free Georgia count`
from `2012 election parl party list - tbilisi district`
;
...
from `2012 election parl party list - districts` as p
inner join (
	select
...
p.`sum logic fail difference`, t.first_result as first_place_result, t.second_result as second_place_result,
(t.first_result - t.second_result - p.`sum logic fail difference`) as difference_first_second_logic_check
from `2012 election parl party list - tbilisi district` as p
inner join (
	select
	t1.district_id, tmax.result as first_result, max(t1.result) as second_result
	from district_results as t1
