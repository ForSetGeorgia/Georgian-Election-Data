...
truncate table `2012 election parl party list - staging`;
truncate table `2012 election parl party list - raw`;

# load data from the csv file
LOAD DATA LOCAL INFILE '/var/www/data/2012_live_party_list.csv'
INTO TABLE `2012 election parl party list - staging`
...
insert into `2012 election parl party list - raw`
select distinct
`rd`.`region` AS `region`,
...
from ((`2012 election parl party list - staging` `staging`
join `regions_districts` `rd` on((`staging`.`district_id` = `rd`.`district_id`)))
join `2012 election parl party list - parties` `parties` on(((`staging`.`district_id` = `parties`.`district_id`) and (`staging`.`precinct_id` = `parties`.`precinct_id`))));
...
INTO OUTFILE '/var/www/data/upload_2012_live_party_list.csv'
FIELDS TERMINATED BY ','
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
