...
truncate table `2014 election local party - raw`;

# load data from the csv file
LOAD DATA INFILE '/var/www/data/2014_official_local_party.csv'
INTO TABLE `2014 election local party - raw`
...
update `2014 election local party - raw` as t, regions_districts as rd
set t.region = rd.region,t.district_name = rd.district_name
where t.district_id = rd.district_id;

# add valid votes
update `2014 election local party - raw`
...
'Georgian Dream'
union
select * from `2014 election local party - csv` where common_id != '' && common_name != ''
INTO OUTFILE '/var/www/data/upload_2014_official_local_party.csv'
FIELDS TERMINATED BY ','
ENCLOSED BY '"'
LINES TERMINATED BY '\n';
