...
def create_table(month)
  table_name = "#{@year} #{month} voters list no tbilisi - raw"
  ...
  sql = "CREATE TABLE IF NOT EXISTS `#{table_name}` (
    ...
    `prec_id_from_data` int(11) DEFAULT NULL,
...
def create_country(month)
  vl_name = "#{@year} #{month} voters list no tbilisi"
...
create_all_for_voters_list('sept')