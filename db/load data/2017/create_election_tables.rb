...
# indicate whether the shapeset should include the custom shapes like tbilisi
@has_custom_shape_levels = true

# indicate whether the shapeset should include region shapes
@has_region_shapes = false

# the symbol that is used to separate the district and precinct in the precinct name
...
# create the table
def create_table(election, parties, is_local_majoritarian=false)
...
def create_invalid_ballots(election, is_local_majoritarian=false)
  ranges = [
    [0,1],
    [1,3],
    [3,5],
...
# create vpm views
def create_vpm(election, is_local_majoritarian=false)
  ranges = [
    [8,12],
    [12,17],
    [17,20]
...
def create_precincts(election, parties, is_local_majoritarian=false)
  major_name = is_local_majoritarian == true ? 'major_' : ''
...
def create_tbilisi_precincts(election, parties, is_local_majoritarian=false)
  major_name = is_local_majoritarian == true ? 'major_' : ''
...
def create_major_districts(election, parties)
  ...
  sql << " from ((((((((`#{election_name} - raw` `raw`
          ...
          "
  if @has_custom_shape_levels
    sql << " where (`raw`.`district_id` between 1 and 10) "
  end
  sql << " #group by `raw`.`region`, `raw`.`district_name`, `raw`.`district_id`, `raw`.`major_district_id`"
...
def create_major_tbilisi_districts(election, parties)
  election_name = "#{@year} election #{election}"
  view_name = "#{election_name} - #{@shapes[:tbilisi_major_district]}"
  @client.query("drop view if exists `#{view_name}`")
  sql = "create view `#{view_name}` as
          select `raw`.`region` AS `region`,
          `raw`.`district_id` AS `district_id`,
...
def create_csv_party_names(election, parties, shape)
  ...
  if has_independent_parties?(parties)
    # if the merged value is 0, reset it to null
    party_sql << " COALESCE( NULLIF( `#{@year} election #{election} - #{shape}`.`#{@initiative_group_merged_name}`, 0 ), null ) AS `#{@initiative_group_csv_name}` "
  end
  return party_sql.join(', ')
end

def create_csv(election, parties, is_local_majoritarian)
  election_name = "#{@year} election #{election}"
  ...
  if @has_custom_shape_levels
    ...
    if is_local_majoritarian
      sql << "(select 'Tbilisi Majoritarian District' AS `@common_headers[0]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`major_district_id` AS `@common_headers[1]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`major_district_Name` AS `@common_headers[2]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`total valid ballots cast` AS `@common_headers[3]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`percent voters voting` AS `@common_headers[4]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num invalid ballots from 0-1%` AS `@common_headers[5]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num invalid ballots from 1-3%` AS `@common_headers[6]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num invalid ballots from 3-5%` AS `@common_headers[7]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num invalid ballots >5%` AS `@common_headers[8]`,
              NULL AS `@common_headers[9]`,
              -- `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts logic fail` AS `@common_headers[10]`,
              -- `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`percent precincts logic fail` AS `@common_headers[11]`,
              -- `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`avg precinct logic fail difference` AS `@common_headers[12]`,
              -- NULL AS `@common_headers[13]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts more ballots than votes` AS `@common_headers[14]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`percent precincts more ballots than votes` AS `@common_headers[15]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`avg precinct difference more ballots than votes` AS `@common_headers[16]`,
              NULL AS `@common_headers[17]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts more votes than ballots` AS `@common_headers[18]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`percent precincts more votes than ballots` AS `@common_headers[19]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`avg precinct difference more votes than ballots` AS `@common_headers[20]`,
              NULL AS `@common_headers[21]`,
              NULL AS `@common_headers[22]`,
              NULL AS `@common_headers[23]`,
              NULL AS `@common_headers[24]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts vpm 8-12 > #{@vpm_limit}` AS `@common_headers[25]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts vpm 12-17 > #{@vpm_limit}` AS `@common_headers[26]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts vpm 17-20 > #{@vpm_limit}` AS `@common_headers[27]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num precincts vpm > #{@vpm_limit}` AS `@common_headers[28]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num_precincts_reported_number` AS `@common_headers[29]`,
              `#{election_name} - #{@shapes[:tbilisi_major_district]}`.`num_precincts_reported_percent` AS `@common_headers[30]`,
      "

      sql << create_csv_party_names(election, parties, @shapes[:tbilisi_major_district])
      sql << " from `#{election_name} - #{@shapes[:tbilisi_major_district]}`)"
...
def precinct_counts(election, is_local_majoritarian=false)
  puts "===================="
  ...
  sql = "CREATE TABLE `#{election_name} - precinct count` (
      `region` VARCHAR(255) NULL DEFAULT NULL,
      `district_id` varchar(10) NOT NULL DEFAULT '0',"

  if is_local_majoritarian
    sql << "`major_district_id` varchar(10) NOT NULL DEFAULT '0',"
  end
  ...
  if is_local_majoritarian
    ...
    @client.query("drop view if exists `#{election_name} - precinct count by #{@shapes[:major_district]}`")
    @client.query("create view `#{election_name} - precinct count by #{@shapes[:major_district]}` as
                  select `district_id` AS `district_id`,`major_district_id` AS `major_district_id`,sum(`num_precincts`) AS `num_precincts`
                  from `#{election_name} - precinct count`
...
# process an election
def create_all_for_election(election, parties, is_local_majoritarian=false)
  puts "===================="
  puts "> #{election}"

  # create precinct count table/views
  puts " - precinct counts tables"
  precinct_counts(election, @is_local_majoritarian)

  # create the table
  puts " - raw table"
  create_table(election, parties, is_local_majoritarian)

  # create invalid ballots views
  puts " - invalid ballots"
  create_invalid_ballots(election, is_local_majoritarian)

  # create vpm views
  puts " - vpm"
  create_vpm(election, is_local_majoritarian)
  ...
  # create precincts view
  puts " - precinct"
  create_precincts(election, parties, is_local_majoritarian)
  ...
  if @has_custom_shape_levels
    ...
    create_tbilisi_precincts(election, parties, is_local_majoritarian)
  end
  ...
  # create csv view
  puts " - csv"
  create_csv(election, parties, is_local_majoritarian)
...
def local_major
  ...
  create_all_for_election(election, parties, @is_local_majoritarian)
end
