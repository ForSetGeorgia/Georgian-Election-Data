class Admin::CoreIndicatorsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @core_indicator.save
        ...
        format.html { redirect_to admin_core_indicator_path(@core_indicator), notice: msg }
...
logger.debug "++++++++ errors = #{@core_indicator.errors.inspect}"
  ...
  def destroy
    ...
    @core_indicator.destroy
		...
		send_status_update(I18n.t('app.msgs.cache_cleared', :action => msg))
    respond_to do |format|
      format.html { redirect_to admin_core_indicators_url }
      format.json { head :ok }
    end
  end
end
