class Shape2 < ActiveRecord::Base
  has_ancestry

  belongs_to :shape_type
  attr_accessible :shape_type_id, :geometry, :en_common_name, :en_common_id, :ka_common_name, :ka_common_id
  ...
  def common_name
	...
	def self.get_shape_name(shape_id)
		return shape_id.nil? ? "" : select("#{I18n.locale}_common_id")
				.where(:id => shape_id).first
	end

	# get the name of the shape (common_id)
	def self.get_shape_no_geometry(shape_id)
		return shape_id.nil? ? "" : select("id, shape_type_id, #{I18n.locale}_common_id, #{I18n.locale}_common_name, ancestry")
					.where(:id => shape_id).first
	end
	...
	def self.get_shapes_by_type(shape_id, shape_type_id, includeGeoData = false)
		if !shape_id.nil? && !shape_type_id.nil?
		  if includeGeoData
			  Shape2.find(shape_id).subtree.where(:shape_type_id => shape_type_id)
			else
			  Shape2.find(shape_id).subtree.select("id, #{I18n.locale}_common_id, #{I18n.locale}_common_name")
				.where(:shape_type_id => shape_type_id)
		  end
		end
	end
    ...
    json = Hash.new()
      ...
      shape_type_name = ShapeTypeTranslation.get_names(shape_type_id)
      ...
      json["features"] = Array.new(shapes.length) {Hash.new}
			...
			shapes.each_with_index do |shape, i|
				...
				json["features"][i]["properties"] = build_json_properties_for_shape(shape, shape_type_name, indicator_id)
			end
		end
		if indicator_id.nil?
    ...
    json = Hash.new()
      ...
      json["features"] = Array.new(shapes.length) {Hash.new}
			...
			shapes.each_with_index do |shape, i|
				...
				json["features"][i]["properties"] = build_json_properties_for_shape(shape, shape_type_name, indicator_type_id, event_id, true)
...
#		puts "+++ time to build summary json: #{Time.now-start} seconds for event #{event_id} and indicator type #{indicator_type_id}"
  ...
  def self.build_json_properties_for_shape(shape, shape_type_name, ind_id, event_id=nil, isSummary = false)
    start = Time.now
    properties = Hash.new
    if !shape.nil?
			...
			properties["shape_type_name"] = shape_type_name.name_singular
      ...
      if !ind_id.nil?
        ...
        if isSummary
  			  data = LiveData2.get_related_indicator_type_data(shape.id, shape.shape_type_id, event_id, ind_id)
        else
    			data = LiveData2.get_related_indicator_data(shape.id, ind_id)
        end
  ...
  def self.csv_header
    ...
    logger.debug "++++event or shape type was not found in spreadsheet"
		  ...
		  logger.debug "++++found event and shape type, get root shape"
              startPhase = Time.now
		          # get the root shape
		          root = Shape2.joins(:shape_translations)
		...
		logger.debug "+++++++ deleting existing root shape and all of its descendants"
		              ...
		              Shape2.delete_all(["id in (?)", root.subtree_ids])
		              root = nil
		          end
		  ...
		  logger.debug "++++adding root shape"
                  shape = Shape2.create :shape_type_id => shape_type.id, :geometry => row[idx_geo].strip
									# add translations
	    ...
	    logger.debug "++++**missing data in row"
			...
			logger.debug "+++++++ deleting existing #{alreadyExists.length} shape record and all of its descendants "
                        alreadyExists.each do |exists|
						              Shape2.destroy_all(["id in (?)", exists.subtree_ids])
                        end
		                    alreadyExists = nil
		                end
				...
				if !event.nil? && !event.shape_id.nil? && !event.shape.nil?
					...
					if !shape_type.nil?
						Shape2.transaction do
							...
							# delete the shapes
							shapes = Shape2.select("id").where(["id in (:shape_ids) and shape_type_id in (:shape_type_ids)",
							...
							error2 = Shape2.delete_all(["id in (?)", shapes.collect(&:id)])
