    ...
    if !shape.nil?
      ...
      if !ind_id.nil?
        ...
        if isSummary
  			  ...
  			  data = LiveDatum.get_related_indicator_type_data(shape.id, shape.shape_type_id, event_id, ind_id, 7)
        else
    			#data = Datum.get_related_indicator_data(shape.id, ind_id)
    			data = LiveDatum.get_related_indicator_data(shape.id, ind_id, 7)
        end
