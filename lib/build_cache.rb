...
#		events = Event.where("id between 1 and 4")
        ...
        is_custom_view = false
              ...
              if is_custom_view
							...
							startPhase = Time.now
							file_name = "#{i} - #{locale}.json"
							File.open("#{Rails.root}/public/json/#{file_name}", 'w') {|f| f.write(app.response.body)}

							puts "=============== "
							puts "=============== time to write json to file #{(Time.now-startPhase)} seconds"
							puts "=============== "
						end
					elsif !indicator_types[0].core_indicators.nil? && !indicator_types[0].core_indicators.empty? &&
								!indicator_types[0].core_indicators[0].indicators.nil? && !indicator_types[0].core_indicators[0].indicators.empty?
	...
	def self.read_files
		...
		x = File.open("#{Rails.root}/public/json/0 - en.json", "r") {|f| f.read()}
		puts "=== length = #{x.length}"
		puts "#{x}"

		puts "============ total time took #{(Time.now - start)} seconds"
	end

	# create cache for all indicators in an event at a shape level
  def self.event_indicator_caches2(event_id, shape_type_id)
    # turn off the active record logging
    old_logger = ActiveRecord::Base.logger
    ActiveRecord::Base.logger = nil
      				...
      				ind_start = Time.now
    					...
    					if is_custom_view
							  app.get "/#{locale}/json/custom_children_shapes/#{event.shape_id}/shape_type/#{shape_type_id}/event/#{event.id}/indicator/#{indicator.id}/custom_view/#{is_custom_view}"
								...
								startPhase = Time.now
								file_name = "#{i} - #{locale}.json"
								File.open("#{Rails.root}/public/json/#{event_id}/#{file_name}", 'w') {|f| f.write(app.response.body)}
